...
AccountsTemplates.configureRoute = function(route, options) {
  ...
  if (layoutType === "blaze-to-react") {
    ...
    if (Package['kadira:react-layout']) {
      ...
      var mount = require('react-mounter');
      if (!mount) {
        throw new Error("useraccounts:flow-routing requires that your project includes kadira:react-layout package or react-mounter npm package.");
      }
    }
